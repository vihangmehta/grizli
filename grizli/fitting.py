...
class GroupFitter(object):
    ...
    def oned_figure(
        ...
        min_sens_show=0.1,
        ylim_percentile=2,
        scale_on_stacked=False,
        show_individual_templates=False,
        apply_beam_mask=True,
        loglam_1d=True,
        trace_limits=None,
        show_contam=False,
        add_label=True,
        ...
        axc : `~matplotlib.axes._subplots.AxesSubplot`
        ...
        beam_models : list
            List of 2D arrays with the model for each beam.  If not None, then
            use these models instead of computing them from the beam objects.

        median_filter_kwargs : dict
            Parameters passed to `~grizli.utils.safe_nanmedian_filter` to
            filter the 2D models before extracting the 1D spectrum.
        ...
        if units.startswith("spline"):
            ...
            if True:
                ...
                Aspl = utils.bspline_templates(
                    tfit["cont1d"].wave[ran], degree=3, df=df, get_matrix=True, log=True
                )

                cspl, _, _, _ = np.linalg.lstsq(
                    Aspl, tfit["cont1d"].flux[ran], rcond=-1
                )

                yspl = tfit["cont1d"].flux * 0.0
                yspl[ran] = Aspl.dot(cspl)

            else:
                spl = UnivariateSpline(
                    tfit["cont1d"].wave[ran], tfit["cont1d"].flux[ran], ext=1
                )
        ...
        for i in range(self.N):
            ...
            if tfit is not None:
                m_i = beam.compute_model(
                    spectrum_1d=sp, is_cgs=True, in_place=False
                ).reshape(beam.sh)
            ...
            if mspl is not None:
                mspl_i = beam.compute_model(
                    spectrum_1d=mspl, is_cgs=True, in_place=False
                ).reshape(beam.sh)
            ...
            try:
                ...
                f_i = beam.compute_model(
                    spectrum_1d=spf, is_cgs=True, in_place=False
                ).reshape(beam.sh)
            ...
            if hasattr(beam, "init_epsf"):  # grizli.model.BeamCutout
                ...
                if m_i is not None:
                    w, flm, erm = beam.beam.optimal_extract(
                        m_i, bin=bin, ivar=beam.ivar * b_mask
                    )
                else:
                    flm = None
                ...
                if mspl is not None:
                    w, flspl, erm = beam.beam.optimal_extract(
                        mspl_i, bin=bin, ivar=beam.ivar * b_mask
                    )

                w, fl, er = beam.beam.optimal_extract(
                    clean, bin=bin, ivar=beam.ivar * b_mask
                )

                w, sens, ers = beam.beam.optimal_extract(
                    f_i, bin=bin, ivar=beam.ivar * b_mask
                )
            ...
            if units.lower() == "njy":
                unit_corr = 1.0 / sens * w ** 2 / 2.99e18 / 1.0e-23 / 1.0e-9  # /pscale
                unit_label = r"$f_\nu$ (nJy)"
            elif units.lower() == "ujy":
                unit_corr = 1.0 / sens * w ** 2 / 2.99e18 / 1.0e-23 / 1.0e-6  # /pscale
                unit_label = r"$f_\nu$ ($\mu$Jy)"
            elif units == "meps":
                unit_corr = 1000.0
                ...
                unit_corr = 1.0 / sens / 1.0e-19  # /pscale
            ...
            f_alpha = 1.0 / (self.Ngrism[grism.upper()]) * 0.8  # **0.5
            ...
            if flm is not None:
                ...
                ymax = np.maximum(
                    ymax, np.percentile((flm + ep)[clip], 100 - ylim_percentile)
                )

                ymin = np.minimum(
                    ymin, np.percentile((flm - er * 0.0)[clip], ylim_percentile)
                )
            else:
                ...
                ymin = np.minimum(ymin, np.percentile((fl - er * 0.0)[clip], 5))
        ...
        for ax in [axc]:  # [axa, axb, axc]:
            ...
            labels = (
                np.arange(np.ceil(wmin / minor / zp1), np.ceil(wmax / minor / zp1))
                * minor
            )
        ...
        if tfit is not None:
            ...
            sp_model = self.optimal_extract(
                m2d, bin=bin, wave=wave, loglam=loglam_1d, trace_limits=trace_limits
            )
        else:
            bg_model = 0.0
            sp_model = 1.0
        ...
        if mspl is not None:
            ...
            sp_spline = self.optimal_extract(
                m2d, bin=bin, wave=wave, loglam=loglam_1d, trace_limits=trace_limits
            )

        sp_data = self.optimal_extract(
            self.scif_mask[: self.Nspec] - bg_model,
            bin=bin,
            wave=wave,
            loglam=loglam_1d,
            trace_limits=trace_limits,
        ...
        # Contamination
        if show_contam:
            sp_contam = self.optimal_extract(
                self.contamf_mask[: self.Nspec],
                bin=bin,
                wave=wave,
                loglam=loglam_1d,
                trace_limits=trace_limits,
        ...
        for g in sp_data:
            ...
            if units.lower() == "njy":
                ...
                unit_corr *= 1 / 2.99e18 / 1.0e-23 / 1.0e-9  # /pscale
            elif units.lower() == "ujy":
                unit_corr = sp_data[g]["wave"] ** 2 / sp_flat[g]["flux"]
                unit_corr *= 1 / 2.99e18 / 1.0e-23 / 1.0e-6  # /pscale
            elif units == "meps":
                unit_corr = 1000.0
            elif units == "eps":
                unit_corr = 1.0
            elif units == "resid":
                unit_corr = 1.0 / sp_flat[g]["flux"]
            elif units == "nresid":
                unit_corr = 1.0 / sp_model[g]["flux"]
            elif units.startswith("spline"):
                unit_corr = 1.0 / sp_spline[g]["flux"]
            else:  # 'flam
                unit_corr = 1.0 / sp_flat[g]["flux"] / 1.0e-19  # /pscale
            ...
            if show_contam:
                ...
                axc.plot(sp_data[g]["wave"][clip] / zp1 / 1.0e4, contam, color="brown")

            if ((tfit is None) & (clip.sum() > 0)) | (scale_on_stacked):
                # Plot limits
                ymax = np.maximum(
                    ymax, np.percentile((flux + ep), 100 - ylim_percentile)
                )
                ymin = np.minimum(ymin, np.percentile((flux - ep), ylim_percentile))
        ...
        if (ymin - 0.2 * ymax < 0) & (1.2 * ymax > 0):
            axc.plot(
                [wmin / zp1, wmax / zp1], [0, 0], color="k", linestyle=":", alpha=0.8
            )

        # Individual templates
        if (
            (tfit is not None)
            & (show_individual_templates > 0)
            & (units.lower() in ["flam", "njy", "ujy"])
        ):

            xt, yt, mt = utils.array_templates(
                tfit["templates"], z=tfit["z"], apply_igm=(tfit["z"] > IGM_MINZ)
            )
            ...
            if units.lower() == "njy":
                unit_corr = xt ** 2 / 2.99e18 / 1.0e-23 / 1.0e-9  # /pscale
            elif units.lower() == "ujy":
                unit_corr = xt ** 2 / 2.99e18 / 1.0e-23 / 1.0e-6  # /pscale
            else:  # 'flam
                unit_corr = 1.0 / 1.0e-19  # /pscale

            tscl = (yt.T * cfit[self.N :]).T / (1 + tfit["z"]) * unit_corr
            t_names = np.array(list(tfit["cfit"].keys()))[self.N :]
            is_spline = np.array(
                [t.split()[0] in ["bspl", "step", "poly"] for t in tfit["cfit"]][
                    self.N :
                ]
            )
            ...
            if is_spline.sum() > 0:
                ...
                axc.plot(xt / 1.0e4, spline_templ, color="k", alpha=0.5)
                for ti in tscl[is_spline, :]:
                    axc.plot(xt / zp1 / 1.0e4, ti, color="k", alpha=0.1)

            for ci, ti, tn in zip(
                cfit[self.N :][~is_spline], tscl[~is_spline, :], t_names[~is_spline]
            ):
                if ci == 0:
                    continue

                if show_individual_templates > 1:
                    axc.plot(xt / zp1 / 1.0e4, ti, alpha=0.6, label=tn.strip("line "))
                else:
                    axc.plot(xt / zp1 / 1.0e4, ti, alpha=0.6)
    ...
    def optimal_extract(
        ...
        **kwargs,
        ...
        if ivar is None:
            ...
            ivar = 1.0 / self.weighted_sigma2_mask
        ...
        if trace_limits is None:
            ...
            trace_mask = (
                (self.yp_trace_mask > trace_limits[0])
                & (self.yp_trace_mask < trace_limits[1])
            )[: self.Nspec]
        ...
        for grism in self.Ngrism:
            ...
            if wave is None:
                if loglam:
                    ran = np.array(lim[:2]) * 1.0e4
                    ran[1] += lim[2] * bin
                    wave_bin = utils.log_zgrid(ran, lim[2] * bin / np.mean(ran))
                else:
                    wave_bin = np.arange(
                        lim[0] * 1.0e4, lim[1] * 1.0e4 + lim[2] * bin, lim[2] * bin
                    )
            else:
                wave_bin = wave

            flux_bin = wave_bin[:-1] * 0.0
            var_bin = wave_bin[:-1] * 0.0
            n_bin = wave_bin[:-1] * 0.0
