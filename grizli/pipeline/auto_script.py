...
#            )
            ...
            # Convert inverse-variance weight to error
            err_data = 1.0 / np.sqrt(wht_data)
            ...
            # Apply err_scale: use explicit phot_err_scale if positive,
            # otherwise use the ERR_SCALE calculated by make_SEP_catalog
            if phot_err_scale > 0:
                err_data *= phot_err_scale
            elif 'ERR_SCALE' in filter_tab.meta:
                # meta values are stored as (value, comment) tuples
                computed_err_scale = filter_tab.meta['ERR_SCALE']
                if isinstance(computed_err_scale, tuple):
                    computed_err_scale = computed_err_scale[0]
                # Apply err_scale if it's positive
                if computed_err_scale > 0:
                    err_data *= computed_err_scale
                    print(f"SEP: Applying err_scale={computed_err_scale:.6f} to auto/iso flux errors")
